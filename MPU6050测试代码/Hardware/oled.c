...
// 自定义：只修改显存，不刷新屏幕（使用 oledfont.h 中的字体）
...
void DrawStringToGRAM(u8 x, u8 y, u8 *str, u8 size1, u8 mode)
{
    while (*str && *str >= ' ' && *str <= '~')
    {
        DrawCharToGRAM(x, y, *str, size1, mode);
        if (size1 == 8) x += 6;
        else if (size1 == 12) x += 6;
        else if (size1 == 16) x += 8;
        else if (size1 == 24) x += 12;
        str++;
    }
}
